...
DREAM_CONSOLIDATE_PROMPT = """你是一个记忆管理专家。你的任务是整理一组同类别的记忆条目，找出冗余、过时或可合并的记忆。

## 你的工作

你会收到一组属于同一类别的记忆条目，每条有 ID、文本内容和最后更新时间。
请仔细分析这些记忆，执行以下操作：

### 1. 合并重复记忆
找出表达相同事实但措辞不同的记忆对。例如：
- "用户 GitHub 用户名是 plhys" 和 "GitHub 用户名: plhys" → 合并
- "决定在A计划现有基础上改造" 和 "A计划改造决定采用在现有基础上改造，不采用 one-api" → 合并（保留更完整的那条）

### 2. 识别过时记忆
如果一条较新的记忆明显取代了一条较旧的记忆，标记旧的为过时。例如：
- 旧: "iflow-memory v1.2.0 已发布" → 新: "iflow-memory v2.0.0 已发布" → 旧的过时了

### 3. 升级模糊记忆
如果多条记忆可以合并成一条更精确、更完整的描述，给出合并后的新文本。
...
{{
  "actions": [
    ...
    {{
      "type": "obsolete",
      "discard_ids": [101],
      "reason": "已被 #202 取代：旧版本号 vs 新版本号"
    }},
    {{
      "type": "upgrade",
      "discard_ids": [301, 302],
      "new_text": "合并后的更完整描述",
      "reason": "两条碎片合并为一条完整记忆"
    }}
  ]
}}
...
- **宁可不动，不要误合**。如果两条记忆只是话题相关但描述的是不同事实，不要合并
- 保留更完整、更具体的那条。"VPN 本机 IP 改为 10.0.1.100" 比 "修改了 VPN 配置" 更好
- 每次最多输出 20 个 action，优先处理最明显的冗余
- 如果没有需要整理的内容，输出 {{"actions": []}}
- ID 必须是记忆条目中实际存在的 ID，不要编造
